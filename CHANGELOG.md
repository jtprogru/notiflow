...
- **The Action now works on a Windows runner.** It never has, in two separate places, and the manifest, the docs and the `2.0.0-alpha.1` entry claiming Windows support were all wrong. Both were found by the end-to-end job below, the first one on its first run, which is the whole argument for having it.
  - Every step reached the action's own tree through `${{ github.action_path }}`, which is substituted into the script text before bash parses it. On Windows the value is `D:\a\notiflow\notiflow`, so bash ate the backslashes as escapes and the first step exited 127 as `D:anotiflownotiflow/scripts/...`. Steps now go through `$GITHUB_ACTION_PATH` and `$RUNNER_TOOL_CACHE`, which carry the same values without a second parsing pass, and a manifest test guards the class.
  - `verify_signature: true` then failed in `install.sh`: cosign reported a certificate subject that visibly matched the identity regexp it had been handed and refused it anyway. cosign is a native Windows binary and the bash calling it is git-bash, which rewrites arguments on their way to one. The conversion is now disabled for that call and the two arguments that genuinely are paths are converted explicitly with `cygpath`.
