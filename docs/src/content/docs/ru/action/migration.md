...
У того же кода был второй отказ на macOS: BSD `iconv` выходит с ненулевым кодом на
неполном хвостовом символе даже с `-c`, поэтому длинное сообщение, обрывающееся на эмодзи,
роняло отправку целиком. В v2 никакого `iconv` нет.
