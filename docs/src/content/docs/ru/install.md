...
Тот же ключ лежит на `keys.openpgp.org` — эту копию стоит предпочесть, её раздаёт не
проект:
...
В любом случае сверьте отпечаток с приведённым выше. Ключ, скачанный тем же каналом, что и
подписанный им файл, — это удобство, а не доказательство: доказательством служит отпечаток,
сверенный с копией из другого источника.
